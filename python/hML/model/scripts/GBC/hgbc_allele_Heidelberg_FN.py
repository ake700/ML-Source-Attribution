...
direction_mapping.update({province: 'West' for province in ['BC', 'AB', 'MB / SK', 'SK', 'MB', 'NT', 'NU', 'YT']})
        ...
        for feat_eng in config['setup']['feat_eng']:
            ...
            for filter_threshold in config['setup']['filter_threshold']:
                ...
                for y_col in config['setup']['y_col']:
                    ...
                    results_dict = hml_gbc_allele(filtered_df, 'Serovar', 'Heidelberg', test_size, graph, db='default', y_col=y_col, dataType='allele', feat_eng=feat_eng, filter_threshold=filter_threshold)
