...
# from HC_PyScripting import RandomBalancingSampler
...
def get_sampler(method, random_state=None, min_class_size=None):
    """
    Returns a sampler object (SMOTE, random upsampling, or SMOTEENN) based on the given method.

    Parameters:
    - method (str): The sampling method. Options:
      'SMOTE', 'random_upsampling', 'SMOTEENN'. Otherwise returns None.
    - random_state (int, optional): Random state for reproducibility.
    - min_class_size (int, optional): Minimum class size used to determine k_neighbors for SMOTE-based methods.

    Returns:
    - sampler: An instance of a sampler from imbalanced-learn or None if method is not recognized.
    """
    if method == 'SMOTE':
...
def feat_eng_data(df, method, cols_remove=None):
    ...
    if method == 'OHE':
        ...
        if method == 'nzv_OHE':
            cols_encode = df.columns.difference(['Key', 'Province', 'Geography', 'Serovar', 'SourceSite', 'CombinedSites', 'CombinedSite2']).tolist()
            return encode_and_concat(df, cols_encode)
        return df
    else:
        return df 
...
def train_eval_model(X_train, X_test, y_train, y_test, pipeline, cv_splits, scoring='f1_macro', param_grid=None):
    """
    ...
    """
    grid_search = GridSearchCV(estimator=pipeline, param_grid=param_grid, scoring=scoring, cv=cv_splits, n_jobs=config['setup']['n_jobs'][0], verbose=1)
