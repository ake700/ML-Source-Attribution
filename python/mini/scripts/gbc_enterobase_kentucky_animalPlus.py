...
config_path = os.path.join(script_dir, '../model_config_best_ebase_kentucky.yml')
...
results_path = config['setup']['models']['GBC']['results_path']

# Set variables for dictionary
filter_prov = config['setup']['variables']['filter_prov']
model_type = config['setup']['models']['GBC']['model_type']
...
column_name = 'Serovar'
seeds = [34, 42, 777]
cols_keep_file = os.path.join(script_dir, '../data/all_data_feat_2df.txt')
...
for seed in seeds:
    ...
    if not df_filtered.empty:
        if column_name != "None":
            db = "Enterobase" if column_name == "Serovar" else "Sero_calling not found"
        ...
        if feat_elim_method in ['RFE', 'RFECV']:
            ...
            if feat_elim_method == 'RFE':
                for proportion in proportions_list:
                    ...
                    if selector:
                        ...
                        metrics, best_estimator, y_pred = train_eval_model(X_train_transformed, X_test_transformed, y_train, y_test, pipeline, cv_splits=n_splits, param_grid=param_grid)
                        ...
                        if feature_importances is not None:
                            ...
                            feat_output_file = os.path.join(feat_base_path, f"features_{db}_{serovar}_{model_type}_{y_col}_{seed}.csv")
            ...
            if feature_importances is not None:
                ...
                feat_output_file = os.path.join(
                    feat_base_path,
                    f"mini_{db}_{serovar}_{model_type}_{y_col}_{seed}.csv"
                )
        ...
        cm_path = os.path.join(cm_base_path, f"cm_{serovar}_{model_type}_{y_col}_{db}_{seed}.csv")
        ...
        output_path = os.path.join(pred_label_base_path, f"pred_{serovar}_{model_type}_{y_col}_{db}_{seed}.csv")
        test_output_df.to_csv(output_path, index=False)

        model_save_path = os.path.join(save_base_path, f"model_{serovar}_{y_col}_{db}_{seed}.joblib")
